...
def reach_donor_view(request):
    search_donor = request.GET.get('search')
    if search_donor:
        donor = models.Donor.objects.filter(Q(address__icontains=search_donor))
        return render(request,'patient/reach_donor.html',{'donor':donor})
    donor = models.Donor.objects.all()   
    # return render(request,'patient/reach_donor.html',{'donor':donor})
   
    return render(request,'donor/reach_donor.html',{'donor':donor})
